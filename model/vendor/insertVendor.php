<?php
	...
	if(isset($_POST['vendorDetailsStatus'])){
		...
		if(isset($fullName) && isset($mobile) && isset($address)) {
			...
			//Validate Full Name
			if(filter_var($fullName, FILTER_SANITIZE_STRING) === 0 || filter_var($fullName, FILTER_SANITIZE_STRING)) {
				// Valid Full Name
			} else {
				// Full Name is wrong
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Please enter a full name.</div>';
				exit();
			}
